...
#define TAG "ti.timezone"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Titimezone_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Titimezone.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TitimezoneBindings::lookupGeneratedInit(
...
static void Titimezone_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Titimezone_getBinding)->GetFunction());
}

static void Titimezone_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TitimezoneBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TitimezoneBinding = {
	"ti.timezone",
	Titimezone_init,
	Titimezone_dispose
};
...
Java_ti_timezone_TitimezoneBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.timezone", &TitimezoneBinding);
	titanium::KrollBindings::addExternalLookup(&(::TitimezoneBindings::lookupGeneratedInit));
}
